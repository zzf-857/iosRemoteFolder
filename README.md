...
- 84 个 Swift Testing fixture 测试和托管测试 target
- PDF、Markdown、TXT、图片、视频、音乐六类独立查看器入口；演示来源的 PDF/Markdown/TXT/图片/音乐已接入受预算真实内容读取，视频仍保留占位流程
...
- D-036/D-037/D-038/D-039 功能闭环切片已实现：`ViewerRegistry` 按 typed metadata/UTType/MIME/扩展名解析，TXT 使用显式 10 MiB 预算和编码探测，PDF/图片使用显式 50 MiB 预算，演示图片通过原生缩放/平移查看器呈现，演示音频通过同一会话读取并由 `AVAudioPlayer` 播放；D-030B 外部 Provider 证据、SwiftData/Keychain 和其他协议仍未提前视为完成

当前阶段：

- D-030B Files 来源配置生命周期已实现，主程构建/84 项测试基线已通过，真实 Files Provider stale/失效 bookmark、授权和恢复语义仍待验证；D-036/D-037/D-038/D-039 已把 TXT/PDF/Markdown/图片/演示音乐推进为受控真实内容路径。本轮不进入 D-032 的 SwiftData/Keychain 迁移。
...
- 视频内容解码、后台音频/Now Playing/队列、阅读位置恢复与生产级缓存
...
当前主程记录位于工作区的 `Project/todolist/alist-media-player/`，实际仓库当前 `main` 包含尚未推送到 `origin/main` 的本地中文 commit；D-024、D-029、D-035、D-028A、D-028B 会话状态门、D-030A、D-036/D-037/D-038/D-039 内容闭环均已完成主程复核，D-030B Files 配置生命周期已实现且主程基线通过，真实 Files Provider 生命周期、视频内容和生产级媒体能力仍待后续工作单。
